...
/**
 * retorna a primeira posição livre
 * ou -1 caso não exista nenhuma
 */
int db_artist_free_place() {
    for(int i=0; i < MAX_ARTISTS; ++i) {
        if (!db_artists[i].in_use) {
            return i;
        }
    }
    return -1;
...
artist_id_t db_artist_add(const char artist_name[], int popularity, const char genre[]) {
    ...
    if (id != -1) {
        return id; // o artista já existe
    }
    id = db_artist_free_place();
    if (id == -1) {
        return -1;
    }
